...
function run() {
  ...
  return { failed: fail ? Number(fail[1]) : -1, passed: pass ? Number(pass[1]) : -1 };
}

const base = run();
if (base.failed !== 0) {
  console.error(`変異させる前からテストが落ちています（fail=${base.failed}）。先に直してください。`);
  process.exit(1);
}
...
for (const m of MUTATIONS) {
  ...
  if (!original.includes(m.from)) {
    results.push({ ...m, status: "対象なし", failed: 0, note: "変異させる箇所が見つかりません（コードが変わった可能性）" });
    continue;
  }
  fs.writeFileSync(file, original.replace(m.from, m.to));
  let res;
  try { res = run(); } finally { fs.writeFileSync(file, original); }
  results.push({ ...m, status: res.failed > 0 ? "検出" : "見逃し", failed: res.failed });
}
...
const md = [
  ...
  `- 変異させる前： ${base.passed} 件PASS ／ 0 件FAIL`,
  `- 変異の数： ${results.length}`,
  `- **検出できた： ${caught} 件**`,
  ...
  "| --- | --- | --- | --- | --- | --- |",
  ...results.map((r, i) => `| ${i + 1} | ${r.name} | ${r.guards} | \`${r.file}\` | ${r.status === "検出" ? "✅ 検出" : r.status === "見逃し" ? "❌ 見逃し" : "— 対象なし"} | ${r.failed} |`),
  "",
  missed.length
