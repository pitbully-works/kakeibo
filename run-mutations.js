...
const MUTATIONS = [
  ...
  { name: "NISAのタイルを古い設定欄へ飛ばす", guards: "NISAの入力口は内訳だけ",
    file: "index.html", from: '<button class="ds blue" data-act="lp-open" data-kind="nisa">',
    to: '<button class="ds blue" data-go="settings" data-focus="f-nisa">' },
  ...
  { name: "打っても自動で保存しない", guards: "保存ボタンを押さなくても消えない",
    file: "index.html", from: "  clearTimeout(autoSaveT); autoSaveT=setTimeout(autoSave,350);",
  ...
  { name: "せっていの自動保存だけやめる", guards: "せっていもその場で保存される",
    file: "index.html", from: '  if(view==="settings"){ saveSettingsQuiet(); return; }',
    to: '  if(false){ saveSettingsQuiet(); return; }' },
  { name: "自動保存で画面を作り直す（入力中の欄から指が外れる）", guards: "打っている間は作り直さない",
    file: "index.html", from: "function autoSave(){", to: "function autoSave(){ render();" },
