...
const MUTATIONS = [
  ...
  { name: "キャッシュの版数を上げ忘れる", guards: "更新が端末に届く",
    file: "sw.js", from: 'const CACHE = "kakeibo-v11";', to: 'const CACHE = "kakeibo-v10";' },
  ...
  { name: "カテゴリ集計にほかの月を混ぜる", guards: "当月だけを数える",
    file: "core.js", from: "      if (!t || t.type !== \"expense\" || monthOf(t.date) !== ym) return;\n      out[t.cat] = (out[t.cat] || 0) + num(t.amount);",
  ...
  { name: "先月ではなく当月と比べる", guards: "先月との比較",
    file: "core.js", from: "    const prev = categorySpend(txs, shiftYm(ym, -1));", to: "    const prev = categorySpend(txs, ym);" },
  ...
  { name: "先月ではなく当月を写す", guards: "写すのは先月の記録",
    file: "core.js", from: "      return isRecurring(t) && monthOf(t.date) === prevYm;",
    to: "      return isRecurring(t) && monthOf(t.date) === ym;" },
  { name: "印の無い支出まで写す", guards: "🔁が付いたものだけ",
    file: "core.js", from: "      return isRecurring(t) && monthOf(t.date) === prevYm;",
    to: "      return t.type === \"expense\" && monthOf(t.date) === prevYm;" },
  { name: "その月に無い日付を作る（2/31）", guards: "月末へ丸める",
    file: "core.js", from: "      const day = Math.min(days, Math.max(1, Number(String(t.date).slice(8, 10)) || 1));",
    to: "      const day = Math.max(1, Number(String(t.date).slice(8, 10)) || 1);" },
  ...
  { name: "給料日より前でも催促する", guards: "給料日を過ぎてから",
    file: "core.js", from: "      if (hint !== null && day >= hint) {", to: "      if (true) {" },
  { name: "はじめての人にも給料を催促する", guards: "履歴が無ければ催促しない",
    file: "core.js", from: "      if (hint !== null && day >= hint) {", to: "      if (day >= (hint === null ? 1 : hint)) {" },
  ...
  { name: "まだ来ていない記録も数える", guards: "未来の日付は数えない",
    file: "core.js", from: "        && validateDateString(t.date) && t.date <= today;", to: "        && validateDateString(t.date);" },
  ...
  { name: "やることが無くてもカードを出す", guards: "空のカードを出さない",
    file: "index.html", from: '  if(!tasks.length) return "";', to: "" },
