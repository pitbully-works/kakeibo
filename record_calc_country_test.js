...
test("記録電卓履歴は表示対象の国で絞る",()=>{
  ...
  assert.match(block,/let historyCountry=sheetState&&sheetState\.country \? Core\.normalizeCountry\(sheetState\.country\) : curCountry\(\)/);
...
test("編集で追加する計算履歴の国は記録本体の国に合わせる",()=>{
  ...
  const rec=block.indexOf("const recCountry = editIdx>=0 ? txCountryOf(state.tx[editIdx]) : Core.normalizeCountry(st.country||curCountry());");
