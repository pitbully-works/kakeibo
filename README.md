...
撮り方を2つ用意しています。読み取った結果は必ず確認画面に出て、その場で直せます。

**🔍 合計だけをアップで撮る（おすすめ）**
「合計 ¥1,240」の行が画面いっぱいになるくらい近づけて撮ります。
候補になる数字がほぼ1つになるうえ、OCRを **数字だけを読むモード**
（`tessedit_char_whitelist`）で動かすので、桁の読み違いがほとんど起きません。

**🧾 レシート全体を撮る**
「合計」「お会計」「ご請求額」といった語の **右側にある数字だけ** を拾います。
「小計」「お預り」「お釣り」「現金」「ポイント」「値引」などの行は最初から候補から外し、
日付・時刻・電話番号・郵便番号も金額と間違えないよう先に取り除きます。

読み取りの解釈は `core.js` の `parseAmount(text, mode)` にあり、
`ocr.test.js` で実際のレシートに近い文字列を使って検証しています。
