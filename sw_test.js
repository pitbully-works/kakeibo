...
/* ---------- 1. キャッシュ名 ---------- */
test("キャッシュ名が新しくなっている（kakeibo-v33）", () => {
  assert.match(swSrc, /const CACHE = "kakeibo-v33";/, "版数が上がっていない");
  for (const old of ["kakeibo-v1", "kakeibo-v2", "kakeibo-v3", "kakeibo-v4", "kakeibo-v5", "kakeibo-v6", "kakeibo-v7", "kakeibo-v8", "kakeibo-v9", "kakeibo-v10", "kakeibo-v11", "kakeibo-v12", "kakeibo-v13", "kakeibo-v14", "kakeibo-v15", "kakeibo-v16", "kakeibo-v17", "kakeibo-v18", "kakeibo-v19", "kakeibo-v20", "kakeibo-v28", "kakeibo-v29", "kakeibo-v30", "kakeibo-v32"]) {
...
test("install で必要な部品が先に取り込まれる", async () => {
  const sw = bootSW({});
  await sw.install();
  const urls = sw.cachedUrls("kakeibo-v33");
...
test("activate で古いキャッシュを削除する", async () => {
  const sw = bootSW({ oldCaches: ["kakeibo-v1", "kakeibo-v2", "kakeibo-v3", "kakeibo-v4", "kakeibo-v5", "kakeibo-v6", "kakeibo-v7", "kakeibo-v8", "kakeibo-v9", "kakeibo-v10", "kakeibo-v11", "kakeibo-v12", "kakeibo-v13", "kakeibo-v14", "kakeibo-v15", "kakeibo-v16", "kakeibo-v17", "kakeibo-v18", "kakeibo-v19", "kakeibo-v20", "kakeibo-v28", "kakeibo-v29", "kakeibo-v30", "kakeibo-v32"] });
  await sw.seedOld();
  assert.equal(sw.cacheNames().length, 24, "前提の古いキャッシュが用意できていない");

  await sw.install();
  await sw.activate();

  const names = sw.cacheNames();
  assert.deepEqual(names, ["kakeibo-v33"], "古いキャッシュが残っている: " + names.join(", "));
});
...
test("画面を取り直したら、その内容をキャッシュに入れ直す", async () => {
  ...
  const stored = await (await sw.caches.open("kakeibo-v33")).match(INDEX);
...
test("index.html を更新すれば、次に開いたとき新しい画面になる", async () => {
  ...
  const cachedFirst = await (await sw.caches.open("kakeibo-v33")).match(INDEX);
...
test("core.js を取り直したら、その内容をキャッシュに入れ直す", async () => {
  ...
  const stored = await (await sw.caches.open("kakeibo-v33")).match(CORE);
