...
/* ---------- 1. キャッシュ名 ---------- */
test("キャッシュ名が新しくなっている（kakeibo-v27）", () => {
  assert.match(swSrc, /const CACHE = "kakeibo-v27";/, "版数が上がっていない");
...
test("install で必要な部品が先に取り込まれる", async () => {
  const sw = bootSW({});
  await sw.install();
  const urls = sw.cachedUrls("kakeibo-v27");
...
test("activate で古いキャッシュを削除する", async () => {
  ...
  assert.deepEqual(names, ["kakeibo-v27"], "古いキャッシュが残っている: " + names.join(", "));
});
...
test("画面を取り直したら、その内容をキャッシュに入れ直す", async () => {
  ...
  const stored = await (await sw.caches.open("kakeibo-v27")).match(INDEX);
...
test("index.html を更新すれば、次に開いたとき新しい画面になる", async () => {
  ...
  const cachedFirst = await (await sw.caches.open("kakeibo-v27")).match(INDEX);
...
test("core.js を取り直したら、その内容をキャッシュに入れ直す", async () => {
  ...
  const stored = await (await sw.caches.open("kakeibo-v27")).match(CORE);
