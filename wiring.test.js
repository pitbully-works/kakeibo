...
test("画面のスクリプトが、存在しない関数を呼んでいない", () => {
  const declared = declaredNames(appSrc);
  const missing = [...calledNames(appSrc)].filter(
...
test("計算コアが、存在しない関数を呼んでいない", () => {
  const declared = declaredNames(coreSrc);
  const missing = [...calledNames(coreSrc)].filter(
    (n) => !declared.has(n) && !KNOWN_GLOBALS.has(n)
  );
  assert.deepEqual(missing, [], `定義が見つからない関数: ${missing.join(", ")}`);
});
